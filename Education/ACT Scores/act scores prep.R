...
act.2008_2012 <- read_csv("Education/ACT Scores/2012_act.csv") %>%
  select("District Name", "HS Name", "ACT Dist Code", "Grad Year", "N", "Avg Comp") %>%
  rename (
  ...
  filter(hsName != "All Schools") %>%
  mutate(aggScore = N * score) %>%
  group_by(districtName, actCode, gradYear) %>%
  summarise(
    N = sum(N, na.rm=TRUE),
    aggScore=sum(aggScore, na.rm=TRUE)
  ) %>%
  ungroup() %>%
  mutate(score=aggScore/N) %>%
  select(-aggScore) %>%
  drop_na(districtName)

act.2013_2017 <- read_csv("Education/ACT Scores/2017_act.csv") %>%
  select("District Name", "HS Name", "ACT Dist Code", "Grad Year", "N", "Avg Comp") %>%
  rename (
    districtName = "District Name",
    hsName = "HS Name",
...
act.2018 <- read_csv("Education/ACT Scores/2018_act.csv") %>%
  select("District Name", "HS Name", "ACT Dist Code", "Grad Year", "N", "Avg Comp") %>%
  rename (
    districtName = "District Name",
    hsName = "HS Name",
