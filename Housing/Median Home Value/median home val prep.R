...
med.home.val.1990 <- read_csv("Housing/Median Home Value/1990_median_home_val.csv") %>%
...
med.home.val.2000 <- read_csv("Housing/Median Home Value/2000_median_home_val.csv") %>%
  mutate(COUNTYA = formatC(COUNTYA, width = 3, flag = "0")) %>%
...
med.home.val.2010 <- read_csv("Housing/Median Home Value/2010_median_home_val.csv") %>%
  mutate(COUNTYA = formatC(COUNTYA, width = 3, flag = "0")) %>%
...
med.home.val.2017 <- read_csv("Housing/Median Home Value/2017_median_home_val.csv") %>%
  mutate(COUNTYA = formatC(COUNTYA, width = 3, flag = "0")) %>%
...
med.home.val.1990_2010 <- full_join(med.home.val.1990, med.home.val.2000, by = c("countyFIPS")) %>%
  ...
  write_csv("Housing/Median Home Value/med_home_val_1990_2010.csv",append=FALSE)